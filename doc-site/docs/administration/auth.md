...
2. **Authentication phase**: For each authorizer in the chain:
    ...
    - If authentication fails → Return 401 Unauthorized (stop processing)
...
3. **Authorization phase**: For each authorizer in the chain:
    ...
    - If authorization fails → Return 401 Unauthorized (stop processing)
...
2. **Authentication phase**: For each authorizer in the chain:
    ...
    - If authentication fails → Return HTTP 401, abort upgrade (stop processing)
    - Store the authentication result
...
**Note**: WebSocket connections authenticate once during upgrade, but authorize each RPC message. This allows long-lived connections while still enforcing authorization checks on every operation.
...
#### Step 2: Configure in Values File

Add the RPC authorization configuration to your `values-customnet.yaml` file:
...
func (h *YourAuthHandler) Authorize(
    ...
    if err := json.Unmarshal([]byte(req.ResultJson), &authResult); err != nil {
        return &prototk.AuthorizeResponse{
            Authorized:   false,
            ErrorMessage: stringPtr("invalid authentication result"),
        }, nil
    }
...
- Both `Authenticate()` and `Authorize()` can return error messages for debugging
