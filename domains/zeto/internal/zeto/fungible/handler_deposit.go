...
func (h *depositHandler) prepareOutputs(ctx context.Context, useNullifiers bool, amount *commontypes.HexUint256, resolvedSender *pb.ResolvedVerifier) ([]*types.ZetoCoin, []*pb.NewState, error) {
	...
	amounts := make([]*commontypes.HexUint256, 2)
	size := 2
	randomIdx := randomSlot(size)
	amounts[randomIdx] = amount
	amounts[size-randomIdx-1] = commontypes.MustParseHexUint256("0x0")
