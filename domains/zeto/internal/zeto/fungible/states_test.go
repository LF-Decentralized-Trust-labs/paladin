...
func TestPrepareInputs(t *testing.T) {
	...
	_, _, _, _, err := prepareInputsForTransfer(ctx, callbacks, coinSchema, false, stateQueryContext, "Alice", []*types.FungibleTransferParamEntry{{Amount: commontypes.Uint64ToUint256(100)}})
	...
	_, _, _, _, err = prepareInputsForTransfer(ctx, callbacks, coinSchema, false, stateQueryContext, "Alice", []*types.FungibleTransferParamEntry{{Amount: commontypes.Uint64ToUint256(200)}})
