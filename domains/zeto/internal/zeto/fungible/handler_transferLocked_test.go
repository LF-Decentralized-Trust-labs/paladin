...
func TestTransferLockedInit(t *testing.T) {
	...
	tx := &types.ParsedTransaction{
		Params: &types.FungibleTransferLockedParams{
			LockedInputs: []*commontypes.HexUint256{commontypes.MustParseHexUint256("0x0c3d1d2996e66d8512c7c3faa4b5f55180fee870190d589a911b6517dc578dba")},
			Delegate:     "delegate1",
...
func TestTransferLockedAssemble(t *testing.T) {
	...
	tx := &types.ParsedTransaction{
		Params: &types.FungibleTransferLockedParams{
			...
			Transfers: []*types.FungibleTransferParamEntry{
				{
					To:     "Alice",
					Amount: commontypes.MustParseHexUint256("0x13"),
				},
			},
		},
		Transaction: txSpec,
	...
	tx.Params.(*types.FungibleTransferLockedParams).Transfers[0].Amount = commontypes.MustParseHexUint256("0x09")
	calls = 0
	_, err = h.Assemble(ctx, tx, req)
...
func TestGenerateMerkleProofsForLockedStates(t *testing.T) {
	...
	addr, err := commontypes.ParseEthAddress("0x1234567890123456789012345678901234567890")
	assert.NoError(t, err)
	inputCoins := []*types.ZetoCoin{
		{
			Salt:   commontypes.MustParseHexUint256("0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff"),
			Owner:  commontypes.MustParseHexBytes("0x1234"),
			Amount: commontypes.MustParseHexUint256("0x0f"),
		},
	}
	ctx := context.Background()
	...
	inputCoins[0].Owner = commontypes.MustParseHexBytes("0x7cdd539f3ed6c283494f47d8481f84308a6d7043087fb6711c9f1df04e2b8025")
	_, _, err = generateMerkleProofs(ctx, h.callbacks, h.stateSchemas.MerkleTreeRootSchema, h.stateSchemas.MerkleTreeNodeSchema, "Zeto_Anon", queryContext, addr, inputCoins, false)
	assert.EqualError(t, err, "PD210054: Failed to create new leaf node. inputs values not inside Finite Field")

	inputCoins[0].Salt = commontypes.MustParseHexUint256("0x042fac32983b19d76425cc54dd80e8a198f5d477c6a327cb286eb81a0c2b95ec")
	calls := 0
	testCallbacks.MockFindAvailableStates = func() (*prototk.FindAvailableStatesResponse, error) {
		defer func() { calls++ }()
		if calls == 0 {
