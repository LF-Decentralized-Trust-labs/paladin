		...
		},
		{
			...
			tokens: []*types.ZetoNFToken{
				types.NewZetoNFToken(
					(*commontypes.HexUint256)(big.NewInt(456)),
					"https://example.com",
					mockPubKey(),
					big.NewInt(123),
				)},
			...
			params: []*types.NonFungibleTransferParamEntry{
				{
					...
					TokenID: (*commontypes.HexUint256)(big.NewInt(456)), // provided tokenID; non-zero
				},
			},
			resolvedVerifiers: []*pb.ResolvedVerifier{
				{
...
func TestMakeNewState(t *testing.T) {
	...
	algoName := "test"
	ownerStr := "owner1"
	validToken := types.NewZetoNFToken((*commontypes.HexUint256)(big.NewInt(456)), "https://example.com", mockPubKey(), big.NewInt(123))
	...
	tests := []struct {
		...
		},
		{
			...
			useNullifiers: false,
			token:         types.NewZetoNFToken((*commontypes.HexUint256)(big.NewInt(456)), "", mockPubKey(), big.NewInt(123)),
			expectErr:     true,
		},
	}
...
func TestZetoNFToken_UnmarshalJSON(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expectError: false,
			expected: &types.ZetoNFToken{
				Salt:    (*commontypes.HexUint256)(big.NewInt(123)),
				URI:     "https://example.com",
				Owner:   commontypes.MustParseHexBytes("0xabcdef"),
				TokenID: (*commontypes.HexUint256)(big.NewInt(456)),
			},
		},
		{
