...
func TestPrepareInputsForTransfer(t *testing.T) {
	...
	param := &types.NonFungibleTransferParamEntry{
		To:      "recipient1",
		URI:     "",
		TokenID: (*commontypes.HexUint256)(big.NewInt(456)),
	}
			...
			params: []*types.NonFungibleTransferParamEntry{
				{
					...
					TokenID: (*commontypes.HexUint256)(big.NewInt(0)),
				},
			},
			expectErr:   true,
			errContains: "PD210114",
...
func TestFormatProvingRequest(t *testing.T) {
	...
	contractAddr := commontypes.MustEthAddress("0xabc123abc123abc123abc123abc123abc123abc1")

	token := types.NewZetoNFToken(
		(*commontypes.HexUint256)(big.NewInt(1)),
		"https://input.com",
		mockPubKey(),
		big.NewInt(2),
	)
	...
	tests := []struct {
		...
		tokenName          string
		queryContext       string
		contractAddr       *commontypes.EthAddress
		expectErr          bool
		errContains        string
...
func TestPrepareState(t *testing.T) {
	...
	validOwnerStr := commontypes.MustParseHexBytes(zetosigner.EncodeBabyJubJubPublicKey(mockPubKey())).String()
...
func dummyEncodeTxData(ctx context.Context, transaction *pb.TransactionSpecification) (commontypes.HexBytes, error) {
	return []byte("txdata"), nil
}

// dummyEncodeTxDataFailed returns an error
func dummyEncodeTxDataFailed(context.Context, *pb.TransactionSpecification) (commontypes.HexBytes, error) {
...
func TestPrepare(t *testing.T) {
	...
	tests := []struct {
		...
		encodeTxFunc  func(context.Context, *pb.TransactionSpecification) (commontypes.HexBytes, error)
	}{
		{
	...
	params := []*types.NonFungibleTransferParamEntry{
		{
			...
			TokenID: (*commontypes.HexUint256)(big.NewInt(123)),
		},
	}
...
func newTransferParam(to string, tokenIDValue int64, uri string) *types.NonFungibleTransferParamEntry {
	return &types.NonFungibleTransferParamEntry{
		To:      to,
		URI:     uri,
		TokenID: (*commontypes.HexUint256)(big.NewInt(tokenIDValue)),
	}
}
