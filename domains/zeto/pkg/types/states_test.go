...
func validOwner() types.HexBytes {
	encoded := zetosigner.EncodeBabyJubJubPublicKey(mockPubKey())
	return types.MustParseHexBytes(encoded)
}
...
func TestZetoCoin_Hash(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			coin: &ZetoCoin{
				Owner:  validOwner(),
				Amount: (*types.HexUint256)(big.NewInt(500)),
				Salt:   (*types.HexUint256)(big.NewInt(123)),
...
func TestNewZetoNFToken(t *testing.T) {
	tokenID := (*types.HexUint256)(big.NewInt(456))
	uri := "https://example.com"
	saltVal := big.NewInt(123)
	tests := []struct {
		name             string
		tokenID          *types.HexUint256
		uri              string
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			expectedOwner := types.MustParseHexBytes(tc.expectedOwnerHex)
			assert.Equal(t, expectedOwner, nft.Owner, "Owner should be set from encoded public key")
			expectedSalt := (*types.HexUint256)(tc.salt)
...
func TestZetoNFToken_UnmarshalJSON(t *testing.T) {
	...
	ownerHex := types.MustParseHexBytes(zetosigner.EncodeBabyJubJubPublicKey(mockPubKey())).String()
	tests := []struct {
		name         string
		jsonStr      string
		expectedURI  string
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.expectErr {
				...
				expectedOwner := types.MustParseHexBytes(zetosigner.EncodeBabyJubJubPublicKey(mockPubKey()))
	...
	tests := []struct {
	...
	}{
		{
			...
			token: &ZetoNFToken{
				Salt:    (*types.HexUint256)(big.NewInt(123)),
				URI:     uri,
				Owner:   types.MustParseHexBytes(zetosigner.EncodeBabyJubJubPublicKey(mockPubKey())),
				TokenID: tokenID,
