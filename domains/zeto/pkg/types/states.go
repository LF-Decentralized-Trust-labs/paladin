...
type ZetoCoin struct {
	Salt   *types.HexUint256 `json:"salt"`
	Owner  types.HexBytes    `json:"owner"`
	Amount *types.HexUint256 `json:"amount"`
	Locked bool              `json:"locked"`
	hash   *types.HexUint256
}

func (z *ZetoCoin) Hash(ctx context.Context) (*types.HexUint256, error) {
	if z.hash == nil {
		...
		z.hash = (*types.HexUint256)(commitment)
	}
	return z.hash, nil
}
...
func NewZetoNFToken(tokenID *types.HexUint256, uri string, publicKey *babyjub.PublicKey, salt *big.Int) *ZetoNFToken {
	return &ZetoNFToken{
		Salt:    (*types.HexUint256)(salt),
		URI:     uri,
		Owner:   types.MustParseHexBytes(zetosigner.EncodeBabyJubJubPublicKey(publicKey)),
		TokenID: tokenID,
	}
}
...
func (z *ZetoNFToken) Hash(ctx context.Context) (*types.HexUint256, error) {
	if z.utxoToken == nil {
	...
	return (*types.HexUint256)(hash), nil
}
...
type MerkleTreeRoot struct {
	...
	RootIndex types.Bytes32 `json:"rootIndex"`
}
...
func (m *MerkleTreeRoot) Hash() (string, error) {
	...
	return types.Bytes32(h.Sum(nil)).HexString(), nil
}
...
type MerkleTreeNode struct {
	RefKey     types.Bytes32  `json:"refKey"`
	Index      types.Bytes32  `json:"index"`
	Type       types.HexBytes `json:"type"`
	LeftChild  types.Bytes32  `json:"leftChild"`
	RightChild types.Bytes32  `json:"rightChild"`
}
