...
func TestBalanceOfExecCall(t *testing.T) {
	...
	t.Run("Single coin state found", func(t *testing.T) {
		...
		res, err := h.ExecCall(ctx, parsedTx, req)
		assert.NoError(t, err)
		assert.Equal(t, `{"balance":"100"}`, res.ResultJson)
	})
	...
	t.Run("Test with large amounts", func(t *testing.T) {
		...
		res, err := h.ExecCall(ctx, parsedTx, req)
		assert.NoError(t, err)
		expected := `{"balance":"` + largeAmount.Int().String() + `"}`
		assert.Equal(t, expected, res.ResultJson)
	})
}

func TestBalanceOfWithDifferentVerifierTypes(t *testing.T) {
	n := &Noto{
		Callbacks:  mockCallbacks,
		coinSchema: &prototk.StateSchema{Id: "coin"},
	}
	...
	t.Run("Test with different algorithm", func(t *testing.T) {
		req := &prototk.ExecCallRequest{
			ResolvedVerifiers: []*prototk.ResolvedVerifier{
				{
					Lookup:       "alice@node1",
					Verifier:     aliceKey.Address.String(),
					Algorithm:    "ECDSA_SECP256R1", // Different algorithm
					VerifierType: verifiers.ETH_ADDRESS,
				},
			},
			StateQueryContext: "query123",
		...
		res, err := h.ExecCall(ctx, parsedTx, req)
		assert.NoError(t, err)
		assert.Equal(t, `{"balance":"50"}`, res.ResultJson)
	})

	t.Run("Test with different verifier type", func(t *testing.T) {
		req := &prototk.ExecCallRequest{
			ResolvedVerifiers: []*prototk.ResolvedVerifier{
				{
					Lookup:       "alice@node1",
					Verifier:     aliceKey.Address.String(),
					Algorithm:    algorithms.ECDSA_SECP256K1,
					VerifierType: "DIFFERENT_TYPE", // Different verifier type
				},
			},
			StateQueryContext: "query123",
		}
