...
type ZetoDomainContracts struct {
	FactoryAddress       *types.EthAddress
	factoryAbi           abi.ABI
	deployedContracts    map[string]*types.EthAddress
...
func deployImplementations(ctx context.Context, rpc rpcclient.Client, deployer string, contracts []zetoDomainContract) (map[string]*types.EthAddress, map[string]abi.ABI, error) {
	deployedContracts := make(map[string]*types.EthAddress)
...
func deployContract(ctx context.Context, rpc rpcclient.Client, deployer string, contract *zetoDomainContract, deployedContracts map[string]*types.EthAddress) (*types.EthAddress, abi.ABI, error) {
...
func getContractSpec(contract *zetoDomainContract, deployedContracts map[string]*types.EthAddress) (*solutils.SolidityBuild, error) {
...
func deployBytecode(ctx context.Context, rpc rpcclient.Client, deployer string, build *solutils.SolidityBuild) (*types.EthAddress, error) {
	var addr string
	rpcerr := rpc.CallRPC(ctx, &addr, "testbed_deployBytecode", deployer, build.ABI, build.Bytecode.String(), types.RawJSON(`{}`))
	if rpcerr != nil {
		return nil, rpcerr
	}
	return types.MustEthAddress(addr), nil
}
...
func registerImpl(ctx context.Context, name string, domainContracts *ZetoDomainContracts, abiFunc *abi.Entry, deployer string, addr *types.EthAddress, tb testbed.Testbed) error {
	if name == "" {
