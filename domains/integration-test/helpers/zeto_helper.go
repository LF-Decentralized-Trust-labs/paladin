...
type ZetoHelper struct {
	...
	Address *commontypes.EthAddress
}
...
func DeployZetoFungible(ctx context.Context, t *testing.T, rpc rpcclient.Client, domainName, controllerName, tokenName string) *ZetoHelperFungible {
	var addr commontypes.EthAddress
...
func DeployERC20(ctx context.Context, rpc rpcclient.Client, deployer, initialOwnerAddr string) (*commontypes.EthAddress, error) {
	build := solutils.MustLoadBuild(erc20ABI)
	params := fmt.Sprintf(`{"initialOwner":"%s"}`, initialOwnerAddr)
	var addr string
	rpcerr := rpc.CallRPC(ctx, &addr, "testbed_deployBytecode", deployer, build.ABI, build.Bytecode.String(), commontypes.RawJSON(params))
	if rpcerr != nil {
		return nil, rpcerr.RPCError()
	}
	return commontypes.MustEthAddress(addr), nil
}

func (z *ZetoHelperFungible) SetERC20(ctx context.Context, tb testbed.Testbed, sender string, erc20Address *commontypes.EthAddress) {
...
func (z *ZetoHelperFungible) Mint(ctx context.Context, to string, amounts []uint64) *DomainTransactionHelper {
	...
	for i, amount := range amounts {
		entries[i] = &types.FungibleTransferParamEntry{
			To:     to,
			Amount: commontypes.Uint64ToUint256(amount),
		}
	}
...
func (z *ZetoHelper) TransferLocked(ctx context.Context, lockedUtxo *commontypes.HexUint256, delegate, to string, amount uint64) *DomainTransactionHelper {
	...
	return NewDomainTransactionHelper(ctx, z.t, z.rpc, z.Address, fn, toJSON(z.t, &types.FungibleTransferLockedParams{
		LockedInputs: []*commontypes.HexUint256{lockedUtxo},
		Delegate:     delegate,
...
func (z *ZetoHelper) Lock(ctx context.Context, delegate *commontypes.EthAddress, amount int) *DomainTransactionHelper {
	...
	return NewDomainTransactionHelper(ctx, z.t, z.rpc, z.Address, fn, toJSON(z.t, &types.LockParams{
		Delegate: delegate,
		Amount:   commontypes.Uint64ToUint256(uint64(amount)),
	}))
}

func (z *ZetoHelper) DelegateLock(ctx context.Context, tb testbed.Testbed, lockedUtxo *commontypes.HexUint256, delegate *commontypes.EthAddress, sender string) {
...
func (z *ZetoHelper) MintERC20(ctx context.Context, tb testbed.Testbed, erc20Address commontypes.EthAddress, amount int64, from, to string) {
...
func (z *ZetoHelper) ApproveERC20(ctx context.Context, tb testbed.Testbed, erc20Address commontypes.EthAddress, amount int64, from string) {
...
func (z *ZetoHelper) Deposit(ctx context.Context, amount int64) *DomainTransactionHelper {
	params := &types.DepositParams{
		Amount: commontypes.Int64ToInt256(amount),
	}
...
func (n *ZetoHelperNonFungible) Transfer(ctx context.Context, to string, tokenID *commontypes.HexUint256) *DomainTransactionHelper {
