...
contract IdentityRegistry {
  ...
  event IdentityRegistered(bytes32 parentIndentityHash, bytes32 identityHash, string name, address owner);

  event IdentityRemoved(bytes32 parentIndentityHash, bytes32 identityHash, string name, address owner);
  ...
  constructor() {
    ...
    Identity memory rootIdentity = Identity(0, new bytes32[](0), "root", msg.sender);
  ...
  function removeIdentity(bytes32 identityHash) public {
    // Ensure identity is not root
    require(identityHash != bytes32(0), "Cannot remove root identity");
    // Ensure identity exists
    require(bytes(identities[identityHash].name).length > 0, "Identity not found");

    // Ensure sender owns the identity
    require(identities[identityHash].owner == msg.sender, "Forbidden");

    // Store identity information for the event
    bytes32 parentIdentityHash = identities[identityHash].parent;
    string memory name = identities[identityHash].name;
    address owner = identities[identityHash].owner;

    // Remove the identity from its parent's children array
    Identity storage parentIdentity = identities[parentIdentityHash];
    for (uint i = 0; i < parentIdentity.children.length; i++) {
      if (parentIdentity.children[i] == identityHash) {
        // Remove the identity from the children array by replacing ith index with last element
        parentIdentity.children[i] = parentIdentity.children[parentIdentity.children.length - 1];
        // Pop last element
        parentIdentity.children.pop();
        break;
      }
    }

    // Remove all properties associated with the identity
    bytes32[] memory propNames = propertyNames[identityHash];
    for (uint i = 0; i < propNames.length; i++) {
      delete properties[identityHash][propNames[i]];
    }

    // Clear the property names array
    delete propertyNames[identityHash];

    // Remove the identity from the identities mapping
    delete identities[identityHash];

    // Emit the IdentityRemoved event
    emit IdentityRemoved(parentIdentityHash, identityHash, name, owner);
  }
  ...
  function makeIdentityHash(bytes32 parentIdentityHash, string memory name) internal pure returns (bytes32) {
    return sha256(abi.encodePacked(parentIdentityHash, name));
  }
}
