...
func with18Decimals(x int64) *types.HexUint256 {
	bx := new(big.Int).Mul(
		big.NewInt(x),
		new(big.Int).Exp(big.NewInt(10), big.NewInt(18), big.NewInt(0)),
	)
	return (*types.HexUint256)(bx)
}

func with10Decimals(x int64) *types.HexUint256 {
	bx := new(big.Int).Mul(
		big.NewInt(x),
...
// func getJSONPropertyAs(jsonData types.RawJSON, name string, toValue any) {
// 	var mapProp map[string]types.RawJSON
...
var _ = Describe("noto/pente - simple", Ordered, func() {
	...
	Context("Noto domain verification", func() {
		...
		var notoContract *types.EthAddress
		...
		var notoCoinSchemaID *types.Bytes32
		...
		penteGroupNodes1and2 := nototypes.PentePrivateGroup{
			Salt:    types.RandBytes32(),                  // unique salt must be shared privately to retain anonymity
			Members: []string{"bob@node1", "sally@node2"}, // these will be salted to establish the endorsement key identifiers
		}

		var penteContract *types.EthAddress
		...
		var erc20StarsAddr *types.EthAddress
		...
		getERC20Balance := func(identity, node string) *types.HexUint256 {
			addr := getEthAddress(ctx, rpc[node], identity, node)
			type ercBalanceOf struct {
				Param0 *types.HexUint256 `json:"0"`
			}
		...
		decodePrivateEVMEvent := func(eventDef *abi.Entry, log *pldapi.PrivateEVMLog) string {
			...
			Expect(err).To(BeNil())
			b, err := types.DefaultJSONFormatOptions.GetABISerializerIgnoreErrors(ctx).SerializeJSONCtx(ctx, cv)
			Expect(err).To(BeNil())
			return string(b)
		}

		It("waits for the receipt logs on node2", func() {
			var penteReceiptJSON types.RawJSON
		...
		var notoTrackerAddr *types.EthAddress
		It("requests the receipt from pente to get the contract address", func() {
		...
		var notoPenteContractAddr *types.EthAddress
...
func getEthAddress(ctx context.Context, rpc pldclient.PaladinClient, identity, node string) types.EthAddress {
	addr, err := rpc.PTX().ResolveVerifier(ctx, fmt.Sprintf("%s@%s", identity, node), algorithms.ECDSA_SECP256K1, verifiers.ETH_ADDRESS)
	Expect(err).To(BeNil())
	return *types.MustEthAddress(addr)
}
