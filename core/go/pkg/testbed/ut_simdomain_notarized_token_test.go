...
type UTXOTransfer_Event struct {
	TX        types.Bytes32   `json:"txId"`
	Inputs    []types.Bytes32 `json:"inputs"`
	Outputs   []types.Bytes32 `json:"outputs"`
	Signature types.HexBytes  `json:"signature"`
}

func parseStatesFromEvent(txID types.Bytes32, states []types.Bytes32) []*prototk.StateUpdate {
...
func TestDemoNotarizedCoinSelection(t *testing.T) {
	...
	fakeCoinDomain := plugintk.NewDomain(func(callbacks plugintk.DomainCallbacks) plugintk.DomainAPI {
			...
			for {
					...
					Statements: query.Statements{
						Ops: query.Ops{
							Eq: []*query.OpSingleVal{
								{Op: query.Op{Field: "owner"}, Value: types.JSONString(fromAddr.String())},
							},
						},
					},
				}
				if lastStateTimestamp > 0 {
					jq.GT = []*query.OpSingleVal{
						{Op: query.Op{Field: ".created"}, Value: types.RawJSON(strconv.FormatInt(lastStateTimestamp, 10))},
					}
				}
				res, err := callbacks.FindAvailableStates(ctx, &prototk.FindAvailableStatesRequest{
					StateQueryContext: stateQueryContext,
					SchemaId:          fakeCoinSchemaID,
					QueryJson:         types.JSONString(jq).String(),
				})
				if err != nil {
		...
		return &plugintk.DomainAPIBase{Functions: &plugintk.DomainAPIFunctions{
			...
			InitContract: func(ctx context.Context, icr *prototk.InitContractRequest) (*prototk.InitContractResponse, error) {
				...
				str := types.HexBytes(icr.ContractConfig).HexString0xPrefix()
			...
			ExecCall: func(ctx context.Context, ecr *prototk.ExecCallRequest) (*prototk.ExecCallResponse, error) {
				...
				return &prototk.ExecCallResponse{
					ResultJson: types.JSONString(&getBalanceResult{
						Amount: (*types.HexUint256)(balance),
					}).Pretty(),
				}, nil
			},
		}}
	...
	rpcErr := tbRPC.CallRPC(ctx, &contractAddr, "testbed_deploy", "domain1", "me", types.RawJSON(`{
		"notary": "domain1.contract1.notary",
		"name": "FakeToken1",
		"symbol": "FT1"
	}`))
	...
	rpcErr = tbRPC.CallRPC(ctx, types.RawJSON{}, "testbed_invoke", &pldapi.TransactionInput{
	...
	rpcErr = tbRPC.CallRPC(ctx, &balance, "testbed_call", &pldapi.TransactionInput{
		...
		ABI: fakeCoinABI,
	}, types.DefaultJSONFormatOptions)
	assert.NoError(t, rpcErr)
...
func deploySmartContract(t *testing.T, confFile string) *types.EthAddress {
	ctx := context.Background()
	tb := NewTestBed()
