...
type EthClient interface {
	...
	GasPrice(ctx context.Context) (gasPrice *types.HexUint256, err error)
	GetBalance(ctx context.Context, address types.EthAddress, block string) (balance *types.HexUint256, err error)
	...
	GetTransactionCount(ctx context.Context, fromAddr types.EthAddress) (transactionCount *types.HexUint64, err error)
...
type EthClientWithKeyManager interface {
	...
	BuildRawTransaction(ctx context.Context, txVersion EthTXVersion, from string, tx *ethsigner.Transaction, opts ...CallOption) (types.HexBytes, error)
}
...
type EstimateGasResult struct {
	GasLimit   types.HexUint64
	RevertData types.HexBytes
}
...
func (cr CallResult) JSON() (s string) {
	if cr.DecodedResult != nil {
		...
		if serializer == nil {
			serializer = types.StandardABISerializer()
		}
...
func (ec *ethClient) resolveFrom(ctx context.Context, from *string, tx *ethsigner.Transaction) (string, *types.EthAddress, error) {
	if from != nil && *from != "" {
		var fromAddr *types.EthAddress
		keyHandle, fromVerifier, err := ec.keymgr.ResolveKey(ctx, *from, algorithms.ECDSA_SECP256K1, verifiers.ETH_ADDRESS)
		if err == nil {
			fromAddr, err = types.ParseEthAddress(fromVerifier)
		}
		if err != nil {
			return "", nil, err
		}
...
func (ec *ethClient) CallContract(ctx context.Context, from *string, tx *ethsigner.Transaction, block string, opts ...CallOption) (res CallResult, err error) {
	if ec.keymgr == nil && from != nil && *from != "" {
		ethAddr, err := types.ParseEthAddress(*from)
		if err != nil {
			return res, err
		}
		tx.From = json.RawMessage(types.JSONString(ethAddr))
	} else {
	...
	if err := ec.rpc.CallRPC(ctx, &res.Data, "eth_call", tx, block); err != nil {
		...
		if len(rpcErr.Data) != 0 {
			...
			if len(res.RevertData) > 0 {
				...
				if errString == "" {
					errString = types.HexBytes(res.RevertData).String()
				}
...
func (ec *ethClient) GetBalance(ctx context.Context, address types.EthAddress, block string) (*types.HexUint256, error) {
	var addressBalance types.HexUint256
...
func (ec *ethClient) GetTransactionCount(ctx context.Context, fromAddr types.EthAddress) (*types.HexUint64, error) {
	var transactionCount types.HexUint64
...
func (ec *ethClient) BuildRawTransaction(ctx context.Context, txVersion EthTXVersion, from string, tx *ethsigner.Transaction, opts ...CallOption) (types.HexBytes, error) {
