...
func testInvokeNewWidgetOk(t *testing.T, isWS bool, txVersion EthTXVersion, gasLimit bool) {
	...
	ctx, ecf, done := newTestClientAndServer(t, &mockEth{
		eth_getTransactionCount: func(ctx context.Context, a types.EthAddress, block string) (types.HexUint64, error) {
		...
		eth_callErr: func(ctx context.Context, req *rpcclient.RPCRequest) *rpcclient.RPCResponse {
			return &rpcclient.RPCResponse{
				...
				Error: &rpcclient.RPCError{
					...
					Message: "reverted",
					Data:    types.JSONString(types.HexBytes(errData)),
				},
			}
		},
	})
...
func TestNewWSOk(t *testing.T) {
	...
	expected := types.RandBytes32()
	ctx, ecf, done := newTestClientAndServer(t, &mockEth{
		eth_sendRawTransaction: func(ctx context.Context, rawTX types.HexBytes) (types.HexBytes, error) {
			return types.HexBytes(expected[:]), nil
		},
	})
