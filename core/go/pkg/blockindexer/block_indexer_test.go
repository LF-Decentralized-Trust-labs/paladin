...
func testBlockArray(t *testing.T, l int, knownAddress ...ethtypes.Address0xHex) ([]*BlockInfoJSONRPC, map[string][]*TXReceiptJSONRPC) {
	...
	for i := 0; i < l; i++ {
		...
		if knownAddress != nil {
			...
			emitAddr1 = ethtypes.MustNewAddress(types.RandHex(20))
		}
		if i == 0 {
		...
		receipts[blocks[i].Hash.String()] = []*TXReceiptJSONRPC{
			{
				...
				Logs: []*LogJSONRPC{
					{Address: emitAddr1, BlockNumber: blocks[i].Number, LogIndex: 0, TransactionHash: txHash, Topics: []ethtypes.HexBytes0xPrefix{topicA, ethtypes.MustNewHexBytes0xPrefix(types.RandHex(32))}},
					{Address: emitAddr1, BlockNumber: blocks[i].Number, LogIndex: 1, TransactionHash: txHash, Topics: []ethtypes.HexBytes0xPrefix{topicB, ethtypes.MustNewHexBytes0xPrefix(types.RandHex(32))}, Data: eventBData},
...
func TestBlockIndexerCatchUpToHeadFromZeroWithConfirmations(t *testing.T) {
	...
	for i := 0; i < len(blocks)-bi.requiredConfirmations; i++ {
		...
		// Get the transaction
		txHash := types.Bytes32(receipts[blocks[i].Hash.String()][0].TransactionHash)
		...
		// Decode events
		decodedEvents, err := bi.DecodeTransactionEvents(ctx, types.Bytes32(tx0.TransactionHash), testABI, "")
...
func TestBlockIndexerListenFromCurrentBlock(t *testing.T) {
	...
	var ch types.HexUint64
	for ch < 9 {
		time.Sleep(10 * time.Millisecond)
		ch, err = bi.GetConfirmedBlockHeight(ctx)
		require.NoError(t, err)
	}
	assert.Equal(t, types.HexUint64(9), ch)
}
...
func TestGetIndexedTransactionByHashErrors(t *testing.T) {
	...
	res, err := bi.GetIndexedTransactionByHash(ctx, types.RandBytes32())
	require.NoError(t, err)
	assert.Nil(t, res)

	p.Mock.ExpectQuery("SELECT.*indexed_transactions").WillReturnError(fmt.Errorf("pop"))

	_, err = bi.GetIndexedTransactionByHash(ctx, types.RandBytes32())
...
func TestWaitForTransactionErrorCases(t *testing.T) {
	...
	_, err := bi.WaitForTransactionSuccess(ctx, types.RandBytes32(), nil)
	assert.Regexp(t, "pop", err)
...
func TestDecodeTransactionEventsFail(t *testing.T) {
	...
	_, err := bi.DecodeTransactionEvents(ctx, types.RandBytes32(), testABI, "")
	assert.Regexp(t, "pop", err)
...
func TestWaitForTransactionSuccessGetReceiptFail(t *testing.T) {
	...
	err := bi.getReceiptRevertError(ctx, types.RandBytes32(), nil)
	assert.Regexp(t, "pop", err)
...
func TestGetIndexedTransactionByNonceFail(t *testing.T) {
	...
	_, err := bi.GetIndexedTransactionByNonce(ctx, types.EthAddress(types.RandBytes(20)), 12345)
	assert.Regexp(t, "pop", err)
