...
func (r *registry) configureEventStream(ctx context.Context, dbTX persistence.DBTX) (err error) {
	...
	for i, es := range r.config.EventSources {
		...
		var contractAddr *types.EthAddress
		if es.ContractAddress != "" {
			contractAddr, err = types.ParseEthAddress(es.ContractAddress)
			if err != nil {
...
func (r *registry) upsertRegistryRecords(ctx context.Context, dbTX persistence.DBTX, protoEntries []*prototk.RegistryEntry, protoProps []*prototk.RegistryProperty) error {
	...
	for i, protoEntry := range protoEntries {
		...
		if err := types.ValidateSafeCharsStartEndAlphaNum(ctx, protoEntry.Name, types.DefaultNameMaxLen, "name"); err != nil {
		...
		if protoEntry.Location != nil {
			txHash, _ := types.ParseBytes32(protoEntry.Location.TransactionHash)
	...
	for i, protoProp := range protoProps {
		...
		entryID, err := types.ParseHexBytes(ctx, protoProp.EntryId)
		if err != nil || len(entryID) == 0 {
		...
		if err := types.ValidateSafeCharsStartEndAlphaNum(ctx, nameToCheck, types.DefaultNameMaxLen, "name"); err != nil {
		...
		if protoProp.Location != nil {
			txHash, _ := types.ParseBytes32(protoProp.Location.TransactionHash)
...
func (r *registry) GetEntryProperties(ctx context.Context, dbTX persistence.DBTX, fActive pldapi.ActiveFilter, entryIDs ...types.HexBytes) ([]*pldapi.RegistryProperty, error) {
...
func filteredPropsMap(entryProps []*pldapi.RegistryProperty, entryID types.HexBytes) map[string]string {
...
func (r *registry) QueryEntriesWithProps(ctx context.Context, dbTX persistence.DBTX, fActive pldapi.ActiveFilter, jq *query.QueryJSON) ([]*pldapi.RegistryEntryWithProperties, error) {
	...
	entryIDs := make([]types.HexBytes, len(entries))
