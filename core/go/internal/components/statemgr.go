...
type StateManager interface {
	...
	NewDomainContext(ctx context.Context, domain Domain, contractAddress types.EthAddress) DomainContext
	...
	GetSchemaByID(ctx context.Context, dbTX persistence.DBTX, domainName string, schemaID types.Bytes32, failNotFound bool) (*pldapi.Schema, error)
	...
	FindStates(ctx context.Context, dbTX persistence.DBTX, domainName string, schemaID types.Bytes32, query *query.QueryJSON, extQueryOptions *StateQueryOptions) (s []*pldapi.State, err error)

	// GetState returns state by ID, with optional labels
	GetStatesByID(ctx context.Context, dbTX persistence.DBTX, domainName string, contractAddress *types.EthAddress, stateIDs []types.HexBytes, failNotFound, withLabels bool) ([]*pldapi.State, error)
...
type DomainContext interface {
	...
	FindAvailableStates(dbTX persistence.DBTX, schemaID types.Bytes32, query *query.QueryJSON) (Schema, []*pldapi.State, error)
	...
	// - Confirmed or not
	// - Spent or not
	GetStatesByID(dbTX persistence.DBTX, schemaID types.Bytes32, ids []string) (Schema, []*pldapi.State, error)
	...
	//
	// The dbTX is passed in to allow re-use of a connection during read operations.
	FindAvailableNullifiers(dbTX persistence.DBTX, schemaID types.Bytes32, query *query.QueryJSON) (Schema, []*pldapi.State, error)
...
type Schema interface {
	...
	ProcessState(ctx context.Context, contractAddress *types.EthAddress, data types.RawJSON, id types.HexBytes, customHash bool) (*StateWithLabels, error)
