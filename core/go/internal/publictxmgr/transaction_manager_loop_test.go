...
func TestNewEnginePollingStoppingAnOrchestratorForFairnessControl(t *testing.T) {
	testSigningAddr1 := types.RandAddress()
	testSigningAddr2 := types.RandAddress()
	...
	ble.inFlightOrchestrators = map[types.EthAddress]*orchestrator{
...
func TestNewEnginePollingExcludePausedOrchestrator(t *testing.T) {
	...
	ble.inFlightOrchestrators = map[types.EthAddress]*orchestrator{}
	ble.signingAddressesPausedUntil = map[types.EthAddress]time.Time{testSigningAddr1: time.Now().Add(1 * time.Hour)}
