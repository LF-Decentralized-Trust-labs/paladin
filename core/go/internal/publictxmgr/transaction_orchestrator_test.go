...
func newTestOrchestrator(t *testing.T, cbs ...func(mocks *mocksAndTestControl, conf *pldconf.PublicTxManagerConfig)) (context.Context, *orchestrator, *mocksAndTestControl, func()) {
	...
	signingAddress := types.EthAddress(types.RandBytes(20))
...
func TestNewOrchestratorLoadsSecondTxAndQueuesBalanceCheck(t *testing.T) {
	...
	m.ethClient.On("GetBalance", mock.Anything, o.signingAddress, "latest").Return(types.Uint64ToUint256(100), nil).Run(func(args mock.Arguments) {
...
func TestOrchestratorTriggerTopUp(t *testing.T) {
	...
	autoFuelingSourceAddr := *types.RandAddress()
	...
	txState.ApplyInMemoryUpdates(ctx, &BaseTXUpdates{
		GasPricing: &pldapi.PublicTxGasPricing{
			GasPrice: types.Int64ToInt256(1000),
		},
	})
	...
	m.ethClient.On("GetBalance", mock.Anything, o.signingAddress, "latest").Return(types.Uint64ToUint256(0), nil)

	// Mock the sufficient balance on the auto-fueling source address, and the nonce assignment
	m.ethClient.On("GetBalance", mock.Anything, autoFuelingSourceAddr, "latest").Return(types.Uint64ToUint256(100*1000), nil)
