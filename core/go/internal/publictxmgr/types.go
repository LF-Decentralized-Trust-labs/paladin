...
type BaseTXUpdates struct {
	...
	// GasLimit          *types.HexUint64 // note this is required for some methods (eth_estimateGas)
	TransactionHash   *types.Bytes32
	FirstSubmit       *types.Timestamp
	LastSubmit        *types.Timestamp
	ErrorMessage      *string
...
type BalanceManager interface {
	...
	GetAddressBalance(ctx context.Context, address types.EthAddress) (*AddressAccount, error)
	NotifyAddressBalanceChanged(ctx context.Context, address types.EthAddress)
}
...
type StatusUpdater interface {
	UpdateSubStatus(ctx context.Context, imtx InMemoryTxStateReadOnly, subStatus BaseTxSubStatus, action BaseTxAction, info *fftypes.JSONAny, err *fftypes.JSONAny, actionOccurred *types.Timestamp) error
}
...
func (sOut *RunningStageContextPersistenceOutput) UpdateSubStatus(action BaseTxAction, info *fftypes.JSONAny, err *fftypes.JSONAny) {
	actionOccurred := types.TimestampNow()
...
type InFlightTransactionStateManager interface {
	...
	AddSubmitOutput(ctx context.Context, txHash *types.Bytes32, submissionTime *types.Timestamp, submissionOutcome SubmissionOutcome, errorReason ethclient.ErrorReason, err error)
	AddSignOutput(ctx context.Context, signedMessage []byte, txHash *types.Bytes32, err error)
