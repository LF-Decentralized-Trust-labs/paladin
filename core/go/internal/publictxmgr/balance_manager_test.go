...
func newTestBalanceManager(t *testing.T, autoFuel bool, cbs ...func(m *mocksAndTestControl, conf *pldconf.PublicTxManagerConfig)) (context.Context, *BalanceManagerWithInMemoryTracking, *pubTxManager, *mocksAndTestControl, func()) {
	ctx, ble, m, done := newTestPublicTxManager(t, false, func(m *mocksAndTestControl, conf *pldconf.PublicTxManagerConfig) {
		if autoFuel {
			...
			autoFuelSourceAddr := types.RandAddress()
...
func TestGetAddressBalance(t *testing.T) {
	...
	m.ethClient.On("GetBalance", mock.Anything, exampleAddr, "latest").Return(types.Uint64ToUint256(balanceOld), nil).Once()

	m.ethClient.On("GetBalance", mock.Anything, exampleAddr, "latest").Return(types.Uint64ToUint256(balanceNew), nil).Once()
...
func expectFuelingEqual(t *testing.T, fuelingTx *pldapi.PublicTx, amountToTransfer uint64, from, to types.EthAddress) {
	assert.Equal(t, from, fuelingTx.From)
	assert.Equal(t, to, *fuelingTx.To)
	assert.Equal(t, *types.Uint64ToUint256(amountToTransfer), *fuelingTx.Value)
}
...
func mockAutoFuelTransactionSubmit(m *mocksAndTestControl, bm *BalanceManagerWithInMemoryTracking, uncachedBalance bool) {
	...
	if uncachedBalance {
		...
		m.ethClient.On("GetBalance", mock.Anything, *bm.sourceAddress, "latest").Return(types.Uint64ToUint256(400), nil).Once()
	}

	// Gas estimate for the auto-fueling TX
	m.ethClient.On("EstimateGasNoResolve", mock.Anything, mock.Anything, mock.Anything).
		Return(ethclient.EstimateGasResult{GasLimit: types.HexUint64(10)}, nil).Once()
}
...
func TestTopUpWithNoAmountModificationWithMultipleFuelingTxs(t *testing.T) {
	...
	testDestAddress := *types.RandAddress()
	...
	m.db.ExpectQuery("SELECT.*public_txns").WillReturnRows(sqlmock.NewRows([]string{"from", `Completed__tx_hash`}).
		AddRow(*bm.sourceAddress, types.RandBytes32()))
	...
	m.ethClient.On("GetBalance", mock.Anything, *bm.sourceAddress, "latest").Return(types.Uint64ToUint256(50), nil).Once()
	...
	m.db.ExpectQuery("SELECT.*public_txns").
		WillReturnRows(sqlmock.NewRows([]string{"from", "to", "value"}).AddRow(
			*bm.sourceAddress, testDestAddress, (*types.HexUint256)(expectedTopUpAmount3),
		))
	m.db.ExpectQuery("SELECT.*public_txns").WillReturnRows(sqlmock.NewRows([]string{"from", "to", "value", `Completed__tx_hash`}).
		AddRow(*bm.sourceAddress, testDestAddress, (*types.HexUint256)(expectedTopUpAmount3), nil /* incomplete */))
...
func TestTopUpFailedDueToSourceBalanceBelowRequestedAmountConcurrencyTest(t *testing.T) {
	...
	// Mock the sufficient balance on the auto-fueling source address, and the nonce assignment
	m.ethClient.On("GetBalance", mock.Anything, *bm.sourceAddress, "latest").Return(types.Uint64ToUint256(400), nil).Once() // called once and then cached
...
func TestTopUpFailedDueToUnableToGetSourceAddressBalance(t *testing.T) {
	...
	// Mock the sufficient balance on the auto-fueling source address, and the nonce assignment
	m.ethClient.On("GetBalance", mock.Anything, *bm.sourceAddress, "latest").Return(types.Uint64ToUint256(0), fmt.Errorf("pop")).Once()
