...
type BalanceManagerWithInMemoryTracking struct {
	...
	destinationAddressesFuelingTracked    map[types.EthAddress]*sync.Mutex // use a map of mutex to achieve concurrent balance tracking for different destination addresses but single threaded tracking for the same address
	trackedFuelingTransactionsMux         sync.RWMutex                     // a mutex to avoid concurrent writes to the trackedFuelingTransactions map
	trackedFuelingTransactions            map[types.EthAddress]*pldapi.PublicTx
	...
	addressBalanceChangedMap    map[types.EthAddress]bool
...
func (af *BalanceManagerWithInMemoryTracking) NotifyAddressBalanceChanged(ctx context.Context, address types.EthAddress) {
...
func (af *BalanceManagerWithInMemoryTracking) GetAddressBalance(ctx context.Context, address types.EthAddress) (*AddressAccount, error) {
	af.addressBalanceChangedMapMux.Lock()
	defer af.addressBalanceChangedMapMux.Unlock()
...
func (af *BalanceManagerWithInMemoryTracking) TransferGasFromAutoFuelingSource(ctx context.Context, destAddress types.EthAddress, value *big.Int) (fuelingTx *pldapi.PublicTx, err error) {
...
func NewBalanceManagerWithInMemoryTracking(ctx context.Context, conf *pldconf.PublicTxManagerConfig, publicTxMgr *pubTxManager) (_ BalanceManager, err error) {
	...
	var autoFuelingSourceAddress *types.EthAddress
	...
	if autoFuelingSource != "" {
		...
		if err == nil {
			autoFuelingSourceAddress, err = types.ParseEthAddress(resolved.Verifier.Verifier)
		}
		if err != nil {
	...
	bm := &BalanceManagerWithInMemoryTracking{
		...
		balanceCache:                       cache.NewCache[types.EthAddress, *big.Int](&conf.BalanceManager.Cache, &pldconf.PublicTxManagerDefaults.BalanceManager.Cache),
		...
		destinationAddressesFuelingTracked: make(map[types.EthAddress]*sync.Mutex),
		trackedFuelingTransactions:         make(map[types.EthAddress]*pldapi.PublicTx),
		addressBalanceChangedMap:           make(map[types.EthAddress]bool),
	}
	return bm, nil
}
