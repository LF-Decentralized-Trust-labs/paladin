...
var mockAddress = tktypes.RandAddress()
...
func TestLoadBlockchainEventListeners(t *testing.T) {
	...
	mockQuery1.Run(func(args mock.Arguments) {
		eventStreamType := args.Get(2).(tktypes.Enum[blockindexer.EventStreamType])
	...
	mockQuery2.Run(func(args mock.Arguments) {
		...
		if assert.Len(t, q.GT, 1) {
			...
			assert.Equal(t, tktypes.JSONString("bel2"), q.GT[0].Value)
		}
	})
...
func TestStartBlockchainEventListeners(t *testing.T) {
	var blockIndexer *componentmocks.BlockIndexer
	_, txm, done := newTestTransactionManager(t, true, func(conf *pldconf.TxManagerConfig, mc *mockComponents) {
		blockIndexer = mc.blockIndexer
		mc.blockIndexer.On("StopEventStream", mock.Anything, mock.Anything).Return(nil).Once()
	})
	defer done()
	...
	blockIndexer.On("StartEventStream", mock.Anything, id).Return(errors.New("pop")).Once()

	// error starting listener
	err := txm.startBlockchainEventListeners()
	assert.ErrorContains(t, err, "pop")

	// success
	blockIndexer.On("StartEventStream", mock.Anything, id).Return(nil).Once()
	err = txm.startBlockchainEventListeners()
	assert.NoError(t, err)
}
...
func TestQueryBlockchainEventListeners(t *testing.T) {
	...
	mockQuery.Run(func(args mock.Arguments) {
		...
		assert.Equal(t, tktypes.JSONString("bel1"), q.Eq[0].Value)
	})
...
func TestHandleEventBatch(t *testing.T) {
	...
	gotError := make(chan bool, 1)
	go func() {
		_, err := el.handleEventBatch(context.Background(), nil)
		require.Error(t, err)
		gotError <- true
	}()
	el.cancelCtx()
