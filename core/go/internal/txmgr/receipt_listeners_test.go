...
func TestE2EReceiptListenerDeliveryLateAttach(t *testing.T) {
	...
	contractAddr1 := types.RandAddress()
	contractAddr2 := types.RandAddress()
	...
	receiptInputs := []*components.ReceiptInput{
		...
		},
		{
			...
			OnChain: types.OnChainLocation{
				...
				TransactionHash:  types.Bytes32(types.RandBytes(32)),
				BlockNumber:      12345,
				TransactionIndex: 20,
				LogIndex:         10,
...
func randOnChain(addr *types.EthAddress) types.OnChainLocation {
	return types.OnChainLocation{
		Type:             types.OnChainTransaction,
		TransactionHash:  types.Bytes32(types.RandBytes(32)),
		BlockNumber:      23456,
		TransactionIndex: 30,
...
func testGapsDomainsForNonAvailableReceipts(t *testing.T, pageSize int) {
	...
	txID5 := uuid.New()
	txID6 := uuid.New()
	missingStateID1 := types.HexBytes(types.RandBytes(32))
	missingStateID2 := types.HexBytes(types.RandBytes(32))
	...
	ctx, txm, done := newTestTransactionManager(t, true,
		func(conf *pldconf.TxManagerConfig, mc *mockComponents) {
			...
			mc.stateMgr.On("GetTransactionStates", mock.Anything, mock.Anything, txID2).
				Return(&pldapi.TransactionStates{
					Unavailable: &pldapi.UnavailableStates{
						Confirmed: []types.HexBytes{missingStateID1},
					},
				}, nil).
				Once()
			...
			mc.stateMgr.On("GetTransactionStates", mock.Anything, mock.Anything, txID3).
				Return(&pldapi.TransactionStates{
					Unavailable: &pldapi.UnavailableStates{
						Spent: []types.HexBytes{missingStateID2},
					},
				}, nil).
				Once()
	...
	err = txm.p.DB().WithContext(ctx).Exec("INSERT INTO states ( id, created, domain_name, contract_address ) VALUES ( ?, ?, ?, ? )",
		missingStateID1, types.TimestampNow(), "domain1", contract1,
	).Error
	require.NoError(t, err)
	...
	err = txm.p.DB().WithContext(ctx).Exec("INSERT INTO states ( id, created, domain_name, contract_address ) VALUES ( ?, ?, ?, ? )",
		missingStateID2, types.TimestampNow(), "domain1", contract1,
	).Error
	require.NoError(t, err)
...
func TestCreateBadListener(t *testing.T) {
	...
	err := txm.CreateReceiptListener(ctx, &pldapi.TransactionReceiptListener{
		Name: "badly-behaved",
		Options: pldapi.TransactionReceiptListenerOptions{
			IncompleteStateReceiptBehavior: types.Enum[pldapi.IncompleteStateReceiptBehavior]("misbehave"),
		},
	})
...
func TestDeleteReceiptListenerFail(t *testing.T) {
	...
	_, err = txm.loadListener(ctx, &persistedReceiptListener{Name: "test1", Filters: types.RawJSON(`{}`), Options: types.RawJSON(`{}`)})
...
func TestCreateListenerBadOptions(t *testing.T) {
	...
	_, err := txm.loadListener(ctx, &persistedReceiptListener{
		Filters: types.RawJSON(`{ !badness`),
	})
	assert.Regexp(t, "PD012233", err)

	_, err = txm.loadListener(ctx, &persistedReceiptListener{
		Filters: types.RawJSON(`{}`),
		Options: types.RawJSON(`{ !badness`),
	})
...
func mockGap(conf *pldconf.TxManagerConfig, mc *mockComponents) {
	...
	txID := uuid.New()
	stateID := types.HexBytes(types.RandBytes(32))
