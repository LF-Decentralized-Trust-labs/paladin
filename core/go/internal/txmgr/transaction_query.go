...
func (tm *txManager) resolveABIReferencesAndCache(ctx context.Context, dbTX persistence.DBTX, txs []*components.ResolvedTransaction) (_ []*components.ResolvedTransaction, err error) {
	abis := make(map[types.Bytes32]*pldapi.StoredABI, len(txs))
	for _, tx := range txs {
...
func (tm *txManager) GetPublicTransactionByNonce(ctx context.Context, from types.EthAddress, nonce types.HexUint64) (*pldapi.PublicTxWithBinding, error) {
...
func (tm *txManager) GetPublicTransactionByHash(ctx context.Context, hash types.Bytes32) (*pldapi.PublicTxWithBinding, error) {
