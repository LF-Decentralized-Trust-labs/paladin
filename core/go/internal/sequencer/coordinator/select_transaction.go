...
func (ts *transactionSelector) SelectNextTransaction(ctx context.Context, event *TransactionStateTransitionEvent) (*transaction.Transaction, error) {
	...
	if event != nil {
		...
		log.L(ctx).Infof("no event to tell us that current assembling sender has changed, leaving current assembling sender unchanged")
		if ts.currentAssemblingSender != nil {
			log.L(ctx).Infof("already have a current assembling sender '%s' for transaction selection, putting them back to the front of the queue", ts.currentAssemblingSender.identity)
			...
			if len(ts.fastQueue) == 0 && len(ts.slowQueue) == 0 {
				for nodeIndex, node := range senderNodePool {
					for _, sender := range senderNodePool[nodeIndex] {
						if len(ts.fastQueue) < ts.numSenders {
							log.L(ctx).Infof("putting sender '%+v' from committee '%s' back to the front of the fast queue", sender, node)
							//ts.fastQueue <- &senderLocator{node: node, identity: sender}
						} else {
							log.L(ctx).Infof("not putting sender '%+v' from committee '%s' back to the front of the slow queue", sender, node)
						}
						if len(ts.slowQueue) < ts.numSenders {
							log.L(ctx).Infof("putting sender '%+v' from committee '%s' back to the front of the slow queue", sender, node)
				...
				log.L(ctx).Infof("already have senders in at least one queue, leaving the queues unchanged")
			}
		}
	}
	...
	getFromSlowQueue := func() *senderLocator {
		log.L(ctx).Infof("getting from slow queue")
		select {
		case sender := <-ts.slowQueue:
			log.L(ctx).Infof("got sender '%s' from slow queue", sender.identity)
			return sender
		default:
			return nil
		}
	...
	nextSender := func() *senderLocator {
		...
		if (ts.timesRoundQueue > 1 && len(ts.slowQueue) != 0) || len(ts.fastQueue) == 0 {
			...
			log.L(ctx).Info("timesRoundQueue > 1 and slow queue is not empty, returning slow queue")
		...
		log.L(ctx).Infof("timesRoundQueue %d, returning fast queue", ts.timesRoundQueue)
		return getFromFastQueue()
	}
	...
	for i := 0; i < ts.numSenders; i++ {
		...
		if sender == nil {
			...
			log.L(ctx).Info("both fast and slow queues are empty, just returning the next pooled transaction")
			for _, senderNodes := range selectableTransactionsMap {
				for _, pooledTx := range senderNodes {
					if pooledTx != nil {
		...
		log.L(ctx).Infof("considering next sender identity '%s', node '%s'", sender.identity, sender.node)

		// MRW TODO - tidy up here
		log.L(ctx).Infof("Selectable transaction identities by node: %d", len(selectableTransactionsMap[sender.node]))
		txn, ok := selectableTransactionsMap[sender.node][sender.identity]
		log.L(ctx).Infof("Selectable transaction for that identity: %+v", txn)

		if txn != nil {
			log.L(ctx).Infof("found transaction %s for sender identity '%s', node '%s'", txn.ID.String(), sender.identity, sender.node)
			if ok {
			...
			log.L(ctx).Infof("no candidate transaction found for sender identity '%s', node '%s'", sender.identity, sender.node)
		}
	}
	log.L(ctx).Error("no TX found")
