...
func (dm *domainManager) registrationIndexer(ctx context.Context, dbTX persistence.DBTX, batch *blockindexer.EventDeliveryBatch) ([]*pldapi.EventWithData, txCompletionsOrdered, error) {
	...
	for _, ev := range batch.Events {
		...
		if ev.SoliditySignature == eventSolSig_PaladinRegisterSmartContract_V0 {
			...
			if err != nil {
				log.L(ctx).Errorf("Registration event for unknown domain event: %s", types.JSONString(ev))
				continue
			}
...
func (d *domain) batchEventsByAddress(ctx context.Context, dbTX persistence.DBTX, batchID string, events []*pldapi.EventWithData) (map[types.EthAddress]*pscEventBatch, error) {

	batches := make(map[types.EthAddress]*pscEventBatch)
...
func (d *domain) handleEventBatch(ctx context.Context, dbTX persistence.DBTX, batch *blockindexer.EventDeliveryBatch) error {
	...
	for addr, batch := range batchesByAddress {
		...
		for _, txCompletionEvent := range res.TransactionsComplete {
			...
			if err == nil {
				txHash, err = types.ParseBytes32(txCompletionEvent.Location.TransactionHash)
			}
			if err != nil {
				return err
			}
			...
			completion := &components.TxCompletion{
				...
				ReceiptInput: components.ReceiptInput{
					...
					ReceiptType:   components.RT_Success,
					OnChain: types.OnChainLocation{
						Type:             types.OnChainEvent, // the on-chain confirmation is an event (even though it's a private transaction we're confirming)
...
func (d *domain) recoverTransactionID(ctx context.Context, txIDString string) (*uuid.UUID, error) {
	txIDBytes, err := types.ParseBytes32Ctx(ctx, txIDString)
	if err != nil {
		return nil, err
	}
...
func (d *domain) handleEventBatchForContract(ctx context.Context, dbTX persistence.DBTX, addr types.EthAddress, batch *pscEventBatch) (*prototk.HandleEventBatchResponse, error) {
	...
	for _, state := range res.NewStates {
		...
		if state.Id != nil {
			id, err = types.ParseHexBytes(ctx, *state.Id)
			if err != nil {
		...
		schemaID, err := types.ParseBytes32(state.SchemaId)
		if err != nil {
		...
		newStates = append(newStates, &components.StateUpsertOutsideContext{
			...
			Data:            types.RawJSON(state.StateDataJson),
		})
...
func (d *domain) prepareIndexRecord(ctx context.Context, txIDStr, stateIDStr string) (uuid.UUID, types.HexBytes, error) {
	txUUID, err := d.recoverTransactionID(ctx, txIDStr)
	if err != nil {
		return uuid.UUID{}, nil, err
	}
	stateID, err := types.ParseHexBytes(ctx, stateIDStr)
	if err != nil {
