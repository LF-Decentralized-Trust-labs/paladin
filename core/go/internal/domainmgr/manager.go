...
func NewDomainManager(bgCtx context.Context, conf *pldconf.DomainManagerConfig) components.DomainManager {
	...
	return &domainManager{
		...
		contractCache:    cache.NewCache[types.EthAddress, *domainContract](&conf.DomainManager.ContractCache, pldconf.ContractCacheDefaults),
	}
}
...
type event_PaladinRegisterSmartContract_V0 struct {
	...
	Config   types.HexBytes   `json:"config"`
}
...
func (dm *domainManager) PostInit(c components.AllComponents) error {
	...
	for name, d := range dm.conf.Domains {
		if _, err := types.ParseEthAddress(d.RegistryAddress); err != nil {
...
func (dm *domainManager) getDomainByAddressOrNil(addr *types.EthAddress) *domain {
	dm.mux.Lock()
	defer dm.mux.Unlock()
	return dm.domainsByAddress[*addr]
}

func (dm *domainManager) GetSmartContractByAddress(ctx context.Context, dbTX persistence.DBTX, addr types.EthAddress) (components.DomainSmartContract, error) {
...
func (dm *domainManager) getSmartContractCached(ctx context.Context, dbTX persistence.DBTX, addr types.EthAddress) (pscLoadResult, *domainContract, error) {
