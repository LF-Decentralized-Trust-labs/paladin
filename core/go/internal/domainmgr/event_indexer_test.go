			...
			Events: []*pldapi.EventWithData{
				{
					...
					Address:           (types.EthAddress)(*tp.d.RegistryAddress()),
...
func TestHandleEventBatch(t *testing.T) {
	batchID := uuid.New()
	txID := uuid.New()
	txIDBytes32 := types.Bytes32UUIDFirst16(txID)
	contract1 := types.RandAddress()
	contract2 := types.RandAddress()
	stateSpent := types.RandHex(32)
	stateRead := types.RandHex(32)
	stateConfirmed := types.RandHex(32)
	stateInfo := types.RandHex(32)
	fakeHash1 := types.RandHex(32)
	fakeSchema := types.RandBytes32()
	event1 := &pldapi.EventWithData{
		...
		IndexedEvent: &pldapi.IndexedEvent{
			...
			TransactionIndex: 20,
			LogIndex:         30,
			TransactionHash:  types.MustParseBytes32(types.RandHex(32)),
			Signature:        types.MustParseBytes32(types.RandHex(32)),
		},
		SoliditySignature: "some event signature 1",
		Data:              types.RawJSON(`{"result": "success"}`),
	}
	...
	td, done := newTestDomain(t, false, goodDomainConf(), mockSchemas(), func(mc *mockComponents) {
		...
		mc.stateStore.On("WriteStateFinalizations", mock.Anything, mock.Anything, []*pldapi.StateSpendRecord{
			{DomainName: "test1", State: types.MustParseHexBytes(stateSpent), Transaction: txID}, // the SpentStates StateUpdate
		}, []*pldapi.StateReadRecord{
			{DomainName: "test1", State: types.MustParseHexBytes(stateRead), Transaction: txID}, // the ReadStates StateUpdate
		}, []*pldapi.StateConfirmRecord{
			{DomainName: "test1", State: types.MustParseHexBytes(stateConfirmed), Transaction: txID}, // the ConfirmedStates StateUpdate
			{DomainName: "test1", State: types.MustParseHexBytes(fakeHash1), Transaction: txID},      // the implicit confirm from the NewConfirmedState
		}, []*pldapi.StateInfoRecord{
			{DomainName: "test1", State: types.MustParseHexBytes(stateInfo), Transaction: txID}, // the InfoStates StateUpdate
		}).Return(nil, nil)

		mc.stateStore.On("WritePreVerifiedStates", mock.Anything, mock.Anything, "test1", []*components.StateUpsertOutsideContext{
			{
				ID:              types.MustParseHexBytes(fakeHash1),
				Data:            types.RawJSON(`{"color": "blue"}`),
		...
		mc.txManager.On("FinalizeTransactions", mock.Anything, mock.Anything, mock.MatchedBy(func(receipts []*components.ReceiptInput) bool {
			...
			assert.Equal(t, types.OnChainEvent, r.OnChain.Type)
	...
	td.tp.Functions.HandleEventBatch = func(ctx context.Context, req *prototk.HandleEventBatchRequest) (*prototk.HandleEventBatchResponse, error) {
		return &prototk.HandleEventBatchResponse{
			SpentStates: []*prototk.StateUpdate{
				{
					Id:            "bad",
					TransactionId: types.RandHex(32),
				},
			},
		}, nil
	}
...
func TestReceiptSorting(t *testing.T) {
	// Note the detail of the sorting code is in types.OnChainLocation
	receiptList := txCompletionsOrdered{
		{ReceiptInput: components.ReceiptInput{OnChain: types.OnChainLocation{Type: types.OnChainEvent, BlockNumber: 1100}}},
		{ReceiptInput: components.ReceiptInput{OnChain: types.OnChainLocation{ /* not onchain */ }}},
		{ReceiptInput: components.ReceiptInput{OnChain: types.OnChainLocation{Type: types.OnChainEvent, BlockNumber: 1000}}},
	}
	sort.Sort(receiptList)
	assert.Equal(t, txCompletionsOrdered{
		{ReceiptInput: components.ReceiptInput{OnChain: types.OnChainLocation{Type: types.NotOnChain}}},
		{ReceiptInput: components.ReceiptInput{OnChain: types.OnChainLocation{Type: types.OnChainEvent, BlockNumber: 1000}}},
		{ReceiptInput: components.ReceiptInput{OnChain: types.OnChainLocation{Type: types.OnChainEvent, BlockNumber: 1100}}},
	}, receiptList)
}
