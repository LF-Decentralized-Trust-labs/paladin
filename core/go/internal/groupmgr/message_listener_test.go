...
func createTestGroups(t *testing.T, ctx context.Context, mc *mockComponents, gm *groupManager, groups ...*pldapi.PrivacyGroupInput) []types.HexBytes {
	...
	ids := make([]types.HexBytes, len(groups))
...
func TestE2EMessageListenerDelivery(t *testing.T) {
	...
	goodRemoteMsg := &pldapi.PrivacyGroupMessage{
		...
		Sent:          types.MustParseTimeString("2021-05-15T19:49:04.123Z"),
		Received:      types.MustParseTimeString("2021-05-15T19:49:04.123Z"), /* will be overridden */
...
func TestDeleteMessageListenerFail(t *testing.T) {
	...
	_, err = gm.loadListener(ctx, &persistedMessageListener{Name: "test1", Filters: types.RawJSON(`{"topic":"(((bad listener"}`), Options: types.RawJSON(`{}`)})
	assert.Regexp(t, "PD012509", err)

	_, err = gm.loadListener(ctx, &persistedMessageListener{Name: "test1", Filters: types.RawJSON(`{}`), Options: types.RawJSON(`{}`)})
...
func mockMessages(count int, mc *mockComponents) {
	...
	for i := 0; i < count; i++ {
		rows = rows.AddRow(
			int64(1000),
			uuid.New(),
			types.JSONString(fmt.Sprintf("message %d", i)),
		)
	}
...
func TestLoadCheckpoint(t *testing.T) {
	...
	mdb.ExpectQuery("SELECT.*message_listener_checkpoints").WillReturnRows(sqlmock.NewRows([]string{
		...
		"listener1", int64(400), types.TimestampNow(),
	))
	err = l.loadCheckpoint()
	require.NoError(t, err)
