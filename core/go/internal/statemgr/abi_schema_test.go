...
func TestStoreRetrieveABISchema(t *testing.T) {
	...
	statesQuery, err := ss.GetStatesByID(ctx, ss.p.NOTX(), as.Persisted().DomainName, contractAddress, []types.HexBytes{state1.ID}, true, true)
	require.NoError(t, err)
...
func TestGetSchemaInvalidJSON(t *testing.T) {
	...
	_, err := ss.GetSchemaByID(ctx, ss.p.NOTX(), "domain1", types.Bytes32Keccak(([]byte)("test")), true)
...
func TestABISchemaProcessStateInvalidType(t *testing.T) {
	...
	require.NoError(t, err)
	_, err = as.ProcessState(ctx, types.RandAddress(), types.RawJSON(`{"field1": 12345}`), nil, false)
	assert.Regexp(t, "PD010103", err)
}
...
func TestABISchemaProcessStateBadValue(t *testing.T) {
	...
	as.tc, err = as.definition.TypeComponentTreeCtx(ctx)
	require.NoError(t, err)
	_, err = as.ProcessState(ctx, types.RandAddress(), types.RawJSON(`{!!! wrong`), nil, false)
...
func TestABISchemaProcessStateMismatchValue(t *testing.T) {
	...
	as.tc, err = as.definition.TypeComponentTreeCtx(ctx)
	require.NoError(t, err)
	_, err = as.ProcessState(ctx, types.RandAddress(), types.RawJSON(`{"field1":{}}`), nil, false)
...
func TestABISchemaProcessStateEIP712Failure(t *testing.T) {
	...
	as.tc, err = as.definition.TypeComponentTreeCtx(ctx)
	require.NoError(t, err)
	_, err = as.ProcessState(ctx, types.RandAddress(), types.RawJSON(`{"field1":"0x753A7decf94E48a05Fa1B342D8984acA9bFaf6B2"}`), nil, false)
...
func TestABISchemaInsertCustomHashNoID(t *testing.T) {
	...
	require.NoError(t, err)
	as.tc = tc
	_, err = as.ProcessState(context.Background(), types.RandAddress(), types.RawJSON(`{}`), nil, true)
...
func TestABISchemaInsertStandardHashMismatch(t *testing.T) {
	...
	require.NoError(t, err)
	_, err = as.ProcessState(context.Background(), types.RandAddress(),
		types.RawJSON(`{}`), types.RandBytes(32), false)
...
func TestABISchemaInsertCustomHashBadData(t *testing.T) {
	...
	require.NoError(t, err)
	as.tc = tc
	_, err = as.ProcessState(context.Background(), types.RandAddress(), types.RawJSON(`{}`), types.RandBytes(32), false)
