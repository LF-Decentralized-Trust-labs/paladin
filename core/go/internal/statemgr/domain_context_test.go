...
func TestUpsertSchemaAndStates(t *testing.T) {
	...
	fakeHash1 := types.HexBytes(types.RandBytes(32))
	fakeHash2 := types.HexBytes(types.RandBytes(32))
...
func TestStateLockErrorsTransaction(t *testing.T) {
	...
	err = dc.AddStateLocks(&pldapi.StateLock{
		Type: types.Enum[pldapi.StateLockType]("wrong"),
	})
...
func TestStateContextMintSpendMint(t *testing.T) {
	...
	tx1states, err := dc.UpsertStates(ss.p.NOTX(),
		&components.StateUpsert{Schema: schemaID, Data: types.RawJSON(fmt.Sprintf(`{"amount": 100, "owner": "0xf7b1c69F5690993F2C8ecE56cc89D42b1e737180", "salt": "%s"}`, types.RandHex(32))), CreatedBy: &transactionID1},
		&components.StateUpsert{Schema: schemaID, Data: types.RawJSON(fmt.Sprintf(`{"amount": 10,  "owner": "0xf7b1c69F5690993F2C8ecE56cc89D42b1e737180", "salt": "%s"}`, types.RandHex(32))), CreatedBy: &transactionID1},
		&components.StateUpsert{Schema: schemaID, Data: types.RawJSON(fmt.Sprintf(`{"amount": 75,  "owner": "0xf7b1c69F5690993F2C8ecE56cc89D42b1e737180", "salt": "%s"}`, types.RandHex(32))), CreatedBy: &transactionID1},
		&components.StateUpsert{Schema: infoSchema, Data: types.RawJSON(fmt.Sprintf(`{"info": "some info", "salt": "%s"}`, types.RandHex(32)))},
	)
	require.NoError(t, err)
	...
	tx3states, err := dc.UpsertStates(ss.p.NOTX(),
		&components.StateUpsert{Schema: schemaID, Data: types.RawJSON(fmt.Sprintf(`{"amount": 35, "owner": "0xf7b1c69F5690993F2C8ecE56cc89D42b1e737180", "salt": "%s"}`, types.RandHex(32))), CreatedBy: &transactionID3},
		&components.StateUpsert{Schema: schemaID, Data: types.RawJSON(fmt.Sprintf(`{"amount": 50, "owner": "0x615dD09124271D8008225054d85Ffe720E7a447A", "salt": "%s"}`, types.RandHex(32))), CreatedBy: &transactionID3},
	)
	require.NoError(t, err)
	...
	tx4states, err := dc.UpsertStates(ss.p.NOTX(),
		&components.StateUpsert{Schema: schemaID, Data: types.RawJSON(fmt.Sprintf(`{"amount": 20, "owner": "0x615dD09124271D8008225054d85Ffe720E7a447A", "salt": "%s"}`, types.RandHex(32))), CreatedBy: &transactionID4},
		&components.StateUpsert{Schema: schemaID, Data: types.RawJSON(fmt.Sprintf(`{"amount": 30, "owner": "0x615dD09124271D8008225054d85Ffe720E7a447A", "salt": "%s"}`, types.RandHex(32))), CreatedBy: &transactionID4},
	)
	require.NoError(t, err)
	...
	checkPostCommit(t, ss, transactionID1, // transaction one
		[]types.HexBytes{},
		[]types.HexBytes{},
		[]types.HexBytes{tx1states[0].ID, tx1states[2].ID, tx1states[2].ID}, // mints these three ...
		[]types.HexBytes{tx1states[3].ID},                                   // and has this info
	)
	checkPostCommit(t, ss, transactionID2, // transaction one
		[]types.HexBytes{},
		[]types.HexBytes{tx1states[1].ID}, // just reads this one
		[]types.HexBytes{},
		[]types.HexBytes{},
	)
	checkPostCommit(t, ss, transactionID3, // transaction three
		[]types.HexBytes{tx1states[1].ID, tx1states[2].ID}, // spends these two ..
		[]types.HexBytes{},
		[]types.HexBytes{tx3states[0].ID, tx3states[1].ID}, // and mints these two
		[]types.HexBytes{},
	)
	checkPostCommit(t, ss, transactionID4, // transaction four,
		[]types.HexBytes{tx3states[1].ID},                  // spends this one ...
		[]types.HexBytes{tx1states[0].ID},                  // reads this one ...
		[]types.HexBytes{tx4states[0].ID, tx4states[1].ID}, // and mints these two
		[]types.HexBytes{},
	)

}

func checkPostCommit(t *testing.T, ss *stateManager, txID uuid.UUID, expectedSpent, expectedRead, expectedConfirmed, expectedInfo []types.HexBytes) {
...
func TestStateContextMintSpendWithNullifier(t *testing.T) {
	...
	nullifier1 := types.HexBytes(types.RandBytes(32))
	nullifier2 := types.HexBytes(types.RandBytes(32))
	data1 := types.RawJSON(fmt.Sprintf(`{"amount": 100, "owner": "0xf7b1c69F5690993F2C8ecE56cc89D42b1e737180", "salt": "%s"}`, types.RandHex(32)))
	data2 := types.RawJSON(fmt.Sprintf(`{"amount": 10,  "owner": "0xf7b1c69F5690993F2C8ecE56cc89D42b1e737180", "salt": "%s"}`, types.RandHex(32)))
...
func TestDomainContextFlushErrorCapture(t *testing.T) {
	...
	data1 := fmt.Sprintf(`{"amount": 100, "owner": "0xf7b1c69F5690993F2C8ecE56cc89D42b1e737180", "salt": "%s"}`, types.RandHex(32))
	tx1 := uuid.New()
...
func TestDCMergeUnFlushedWhileFlushing(t *testing.T) {
	...
	s1, err := schema.ProcessState(ctx, contractAddress, types.RawJSON(fmt.Sprintf(
		`{"amount": 20, "owner": "0x615dD09124271D8008225054d85Ffe720E7a447A", "salt": "%s"}`,
		types.RandHex(32))), nil, dc.customHashFunction)
	require.NoError(t, err)
	tx1 := uuid.New()
	...
	err = dc.UpsertNullifiers(&components.NullifierUpsert{ID: types.RandBytes(32), State: s1.ID})
	require.NoError(t, err)
...
func TestDSIMergeUnFlushedMultipleSchemas(t *testing.T) {
	...
	s1, err := schema1.ProcessState(ctx, contractAddress, types.RawJSON(fmt.Sprintf(
		`{"amount": 20, "owner": "0x615dD09124271D8008225054d85Ffe720E7a447A", "salt": "%s"}`,
		types.RandHex(32))), nil, dc.customHashFunction)
	require.NoError(t, err)
	s2, err := schema2.ProcessState(ctx, contractAddress, types.RawJSON(fmt.Sprintf(
		`{"tokenUri": "%s", "owner": "0x615dD09124271D8008225054d85Ffe720E7a447A", "salt": "%s"}`,
		types.RandHex(32), types.RandHex(32))), nil, dc.customHashFunction)
	require.NoError(t, err)
...
func TestDSIMergeUnFlushedBadDBRecord(t *testing.T) {
	...
	_, err = dc.mergeUnFlushedApplyLocks(schema1, []*pldapi.State{
		{StateBase: pldapi.StateBase{ID: types.RandBytes(32), Data: types.RawJSON("wrong")}},
	}, query.NewQueryBuilder().Sort(".created").Query(), true, false)
...
func TestDCMergeUnFlushedWhileFlushingDedup(t *testing.T) {
	...
	s2, err := schema.ProcessState(ctx, contractAddress, types.RawJSON(fmt.Sprintf(
		`{"amount": 20, "owner": "0x615dD09124271D8008225054d85Ffe720E7a447A", "salt": "%s"}`,
		types.RandHex(32))), nil, dc.customHashFunction)
	require.NoError(t, err)
...
func TestDCFindBadQueryAndInsertBadValue(t *testing.T) {
	...
	_, err = dc.UpsertStates(ss.p.NOTX(), &components.StateUpsert{
		Schema: schemaID, Data: types.RawJSON(`"wrong"`),
	})
...
func TestCheckEvalGTTimestamp(t *testing.T) {
	...
	stateID := types.MustParseHexBytes("2eaf4727b7c7e9b3728b1344ac38ea6d8698603dc3b41d9458d7c011c20ce672")

	// create time is equal - no match
	created := types.TimestampFromUnix(1726545933211347000)
	...
	created = types.TimestampFromUnix(1726545933211347001)
	addStateBaseLabels(ls, stateID, created)
	match, err = filters.EvalQuery(ctx, jq, labelSet, ls)
	assert.NoError(t, err)
	assert.True(t, match)
	...
	created = types.TimestampFromUnix(1726545933211346999)
	addStateBaseLabels(ls, stateID, created)
	match, err = filters.EvalQuery(ctx, jq, labelSet, ls)
	assert.NoError(t, err)
	assert.False(t, match)
...
func TestImportSnapshot(t *testing.T) {
	...
	s2, err := schema1.ProcessState(ctx, contractAddress, types.RawJSON(fmt.Sprintf(
		`{"amount": 20, "owner": "0x615dD09124271D8008225054d85Ffe720E7a447A", "salt": "%s"}`,
		types.RandHex(32))), nil, dc.customHashFunction)
	require.NoError(t, err)

	s3ID := types.RandHex(32)

	s4, err := schema1.ProcessState(ctx, contractAddress, types.RawJSON(fmt.Sprintf(
		`{"amount": 20, "owner": "0x615dD09124271D8008225054d85Ffe720E7a447A", "salt": "%s"}`,
		types.RandHex(32))), nil, dc.customHashFunction)
	require.NoError(t, err)

	s5, err := schema1.ProcessState(ctx, contractAddress, types.RawJSON(fmt.Sprintf(
		`{"amount": 20, "owner": "0x615dD09124271D8008225054d85Ffe720E7a447A", "salt": "%s"}`,
		types.RandHex(32))), nil, dc.customHashFunction)
	require.NoError(t, err)
	...
	jsonToImport := fmt.Sprintf(`{
		...
		"states": `+types.JSONString(stateUpserts).Pretty()+`
	}`,
...
func TestGetStatesByIDFail(t *testing.T) {
	...
	_, _, err := dc.GetStatesByID(dc.ss.p.NOTX(), types.Bytes32(types.RandBytes(32)), []string{types.RandHex(32)})
	assert.Regexp(t, "pop", err)
}
