...
func (ss *stateManager) NewDomainContext(ctx context.Context, domain components.Domain, contractAddress types.EthAddress) components.DomainContext {
	id := uuid.New()
...
func (dc *domainContext) getUnFlushedSpends() (spending []types.HexBytes, nullifiers []*pldapi.StateNullifier, nullifierIDs []types.HexBytes, err error) {
	...
	nullifierIDs = make([]types.HexBytes, len(nullifiers))
...
func (dc *domainContext) GetStatesByID(dbTX persistence.DBTX, schemaID types.Bytes32, ids []string) (components.Schema, []*pldapi.State, error) {
...
func (dc *domainContext) FindAvailableStates(dbTX persistence.DBTX, schemaID types.Bytes32, query *query.QueryJSON) (components.Schema, []*pldapi.State, error) {
...
func (dc *domainContext) FindAvailableNullifiers(dbTX persistence.DBTX, schemaID types.Bytes32, query *query.QueryJSON) (components.Schema, []*pldapi.State, error) {
	...
	statesWithNullifiers := make([]types.HexBytes, len(nullifiers))
...
type exportableStateLock struct {
	State       types.HexBytes                   `json:"stateId"`
	Transaction uuid.UUID                        `json:"transaction"`
	Type        types.Enum[pldapi.StateLockType] `json:"type"`
}
