...
func (tf *transactionFlow) setTransactionSigner(ctx context.Context) (reDelegate bool, err error) {
	...
	node, err := types.PrivateIdentityLocator(endorserSubmitSigner).Node(ctx, false /* must be fully qualified in this scenario */)
	if err != nil {
...
func (tf *transactionFlow) delegateIfRequired(ctx context.Context) (doContinue bool) {
	...
	fullQualifiedFrom, err := types.PrivateIdentityLocator(tf.transaction.PreAssembly.TransactionSpecification.From).FullyQualified(ctx, tf.nodeName)
	if err != nil {
...
func (tf *transactionFlow) requestAssemble(ctx context.Context) {
	...
	if err == nil {
		assemblingNode, err = types.PrivateIdentityLocator(preAssemblyCopy.TransactionSpecification.From).Node(ctx, true)
	}
	if err != nil {
...
func (tf *transactionFlow) requestSignature(ctx context.Context, attRequest *prototk.AttestationRequest, partyName string) {
	...
	signerNode, err := types.PrivateIdentityLocator(partyName).Node(ctx, true)
