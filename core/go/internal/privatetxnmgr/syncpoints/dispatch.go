...
type DispatchPersisted struct {
	...
	PublicTransactionAddress types.EthAddress `json:"publicTransactionAddress"`
...
func (s *syncPoints) PersistDispatchBatch(dCtx components.DomainContext, contractAddress types.EthAddress, dispatchBatch *DispatchBatch, stateDistributions []*components.StateDistribution, preparedTxnDistributions []*components.PreparedTransactionWithRefs) error {
	...
	for _, preparedTxnDistribution := range preparedTxnDistributions {
		node, _ := types.PrivateIdentityLocator(preparedTxnDistribution.Transaction.From).Node(dCtx.Ctx(), false)
		if node != s.transportMgr.LocalNodeName() {
			preparedReliableMsgs = append(preparedReliableMsgs, &pldapi.ReliableMessage{
				...
				Metadata:    types.JSONString(preparedTxnDistribution),
			})
		} else {
	...
	for _, stateDistribution := range stateDistributions {
		node, _ := types.PrivateIdentityLocator(stateDistribution.IdentityLocator).Node(dCtx.Ctx(), false)
		preparedReliableMsgs = append(preparedReliableMsgs, &pldapi.ReliableMessage{
			Node:        node,
			MessageType: pldapi.RMTState.Enum(),
			Metadata:    types.JSONString(stateDistribution),
		})
	}
