...
func (p *privateTxManager) getSequencerForContract(ctx context.Context, dbTX persistence.DBTX, contractAddr types.EthAddress, domainAPI components.DomainSmartContract) (oc *Sequencer, err error) {
...
func (p *privateTxManager) getEndorsementGathererForContract(ctx context.Context, dbTX persistence.DBTX, contractAddr types.EthAddress) (ptmgrtypes.EndorsementGatherer, error) {
...
func (p *privateTxManager) revertDeploy(ctx context.Context, tx *components.PrivateContractDeploy, err error) error {
	...
	tryFinalize = func() {
		p.syncPoints.QueueTransactionFinalize(ctx, tx.Domain, types.EthAddress{}, tx.ID, deployError.Error(),
...
func (p *privateTxManager) evaluateDeployment(ctx context.Context, domain components.Domain, tx *components.PrivateContractDeploy) error {
	...
	identifier, node, err := types.PrivateIdentityLocator(tx.Signer).Validate(ctx, p.nodeName, true)
	if err != nil {
		return err
	}
	if node != p.nodeName {
	...
	if tx.InvokeTransaction != nil {
		...
		publicTXs[0].Data = types.HexBytes(data)
